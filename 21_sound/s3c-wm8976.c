...
static DECLARE_WAIT_QUEUE_HEAD(wm8976_waitq_r);
static DECLARE_WAIT_QUEUE_HEAD(wm8976_waitq_w);
...
typedef struct {
	...
	enum dma_ch dma_ch; /* DMA channel (channel2 for audio) */
	u_int dma_ok;
} audio_stream_t;
...
#ifdef DEFINE_POLL
static unsigned int smdk2410_audio_poll(struct file *file, struct poll_table *wait) {
	unsigned int mask = 0;
	int i;
...
static long smdk2410_mixer_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
	int ret;
	long val = 0;
...
static long smdk2410_audio_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
	long val;
	switch (cmd) {
		...
		case SNDCTL_DSP_SETDUPLEX:
			return -ENOSYS;
		default:
			return smdk2410_mixer_ioctl(file, cmd, arg);
	}
...
static int __init audio_init_dma(audio_stream_t *s, char *desc) {
	...
	if (s->dma_ch == DMACH_I2S_OUT) {
		...
		devaddr = 0x55000010;
		//dcon    = S3C2410_DCON_TSZUNIT | S3C2410_DCON_SSERVE; // VAL: 0xa0800000;
		flags   = S3C2410_DMAF_AUTOSTART;

		ret = s3c2410_dma_request(s->dma_ch, &s3c2410iis_dma_out, NULL);
		s3c2410_dma_devconfig(channel, source, devaddr);
		...
		source  = DMA_FROM_DEVICE;
		devaddr = 0x55000010;
		//dcon    = S3C2410_DCON_TSZUNIT | S3C2410_DCON_SSERVE; // VAL: 0xa2800000;
		flags   = S3C2410_DMAF_AUTOSTART;
...
static int s3c2410iis_probe(struct device *dev) {
	...
	s3c2410_gpio_pullup(S3C2410_GPB(4), 1);
	/* GPB 3: L3DATA, OUTPUT */
	s3c2410_gpio_cfgpin(S3C2410_GPB(3), S3C2410_GPIO_OUTPUT);
	/* GPB 2: L3MODE, OUTPUT */
	s3c2410_gpio_cfgpin(S3C2410_GPB(2), S3C2410_GPIO_OUTPUT);
	s3c2410_gpio_pullup(S3C2410_GPB(2), 1);
	/* GPE 3: I2SSDI */
	s3c2410_gpio_cfgpin(S3C2410_GPE(3), S3C2410_GPE3_I2SSDI);
	s3c2410_gpio_pullup(S3C2410_GPE(3), 0);
	/* GPE 0: I2SLRCK */
	s3c2410_gpio_cfgpin(S3C2410_GPE(0), S3C2410_GPE0_I2SLRCK);
	s3c2410_gpio_pullup(S3C2410_GPE(0), 0);
	/* GPE 1: I2SSCLK */
	s3c2410_gpio_cfgpin(S3C2410_GPE(1), S3C2410_GPE1_I2SSCLK);
	s3c2410_gpio_pullup(S3C2410_GPE(1), 0);
	/* GPE 2: CDCLK */
	s3c2410_gpio_cfgpin(S3C2410_GPE(2), S3C2410_GPE2_CDCLK);
	s3c2410_gpio_pullup(S3C2410_GPE(2), 0);
	/* GPE 4: I2SSDO */
	s3c2410_gpio_cfgpin(S3C2410_GPE(4), S3C2410_GPE4_I2SSDO);
	s3c2410_gpio_pullup(S3C2410_GPE(4), 0);
